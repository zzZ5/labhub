...
test('门户业务内容为空时不注入品牌和研究方向兜底文案', async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== 'desktop', 'brand fallback behavior checked once')
  await installMockApi(page)
  const emptyJson = (route: Route) => route.fulfill({
    status: 200,
    contentType: 'application/json',
    body: '{}',
  })
  const emptyList = (route: Route) => route.fulfill({
    status: 200,
    contentType: 'application/json',
    body: '[]',
  })
  await page.route('**/api/portal/site-setting/', emptyJson)
  await page.route('**/api/portal/contact/', emptyJson)
  await page.route('**/api/cms/site-settings/', emptyList)
  await page.route('**/api/cms/contact-info/', emptyList)

  await page.goto('/')
  await expect(page.getByText('中农雨磷', { exact: true })).toHaveCount(0)
  await expect(page.getByText('农业生态环境过程', { exact: true })).toHaveCount(0)

  await page.goto('/cms')
  await expect(page.getByLabel('实验室名称')).toHaveValue('')
  await expect(page.getByLabel('归属单位')).toHaveValue('')
  await expect(page.getByLabel('横幅副标题')).toHaveValue('')
  await expect(page.getByLabel('模块标题')).toHaveValue('')
})
...
test('论文、专利和获奖附件可查看，详情返回恢复列表滚动位置', async ({ page }, testInfo) => {
  ...
  await page.getByRole('tab', { name: '专利', exact: true }).click()
  await page.getByText('一种堆肥微生物调控方法').click()
  await expect(page.getByRole('link', { name: '在线查看 PDF' })).toBeVisible()
  await page.getByRole('link', { name: '返回科研成果' }).click()

  await page.getByRole('tab', { name: '获奖', exact: true }).click()
...
test('仪器搜索、分页、详情返回和编辑入口可用', async ({ page }) => {
  ...
  await instrumentDialog.getByRole('button', { name: '创建设备' }).click()
...
test('成员管理可创建账号并为学生账号生成唯一档案', async ({ page }) => {
  ...
  const drawer = page.locator('.el-drawer').filter({ hasText: '编辑账号' })
